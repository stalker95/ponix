		...
		<div class="row">
			<div class="col-md-3">
				<div class="search_form blog_search">
				    <?= $this->Form->create('', ['type' => 'file','method' => 'get', 'id' => '', 'controller' => 'blog', 'action' => 'search'] )  ?>
				...
				<div class="blog_item">
					...
					<div class="blog_item_description">
						<p><?php 
	          				...
	          				$first_string = substr($value['text'], 700);

	          				$first_empty = strpos($first_string, ' ');

	          			}
	          				if (strlen($value['text']) > 700){ ?>
	          					<?= substr($value['text'], 0, 700 + $first_empty);
					...
					<div class="blog_item_link">
						<a href="<?= $this->Url->build(['controller' => 'blog','action'    =>  $value['slug']], ['fullBase' => true]) ?>">Детальніше</a>
					</div>
